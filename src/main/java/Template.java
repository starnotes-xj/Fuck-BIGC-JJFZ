...
public class Template {
    private static final String baseUrl = "http://xscdx.bigc.edu.cn/jjfz/lesson";
    ...
    private static List<WebElement> getWebElementsList(String ulCssSelector) {
    ...
    private void manageVideo(int index, @NotNull List<WebElement> necessaryList) {
        ...
        // 获取侧边栏视频列表
        List<WebElement> sidebars = getWebElementsList(sidebarCssSelector);
        int times = sidebars.size();
        System.out.print("成功加载侧边栏，共" + sidebars.size() + "个视频");
        boolean isNoVoice = false;
        for (int i = 0; i < times; i++) {
            // 每次循环都重新获取当前视频元素
            sidebars = getWebElementsList(sidebarCssSelector);
    ...
    public void run() {
         ...
         */
        List<WebElement> CoursesList = getWebElementsList(allCourseCssSelector);
        int times = CoursesList.size();
        //循环学习课程
        for (indexStudyList = 0; indexStudyList < times; indexStudyList++) {
            CoursesList = getWebElementsList(allCourseCssSelector);
            ...
            List<WebElement> necessaryList = getWebElementsList(
                    "body > div > div.w1150 > div.wrap_right > div.lesson1_cont.q_lesson1_cont > div.lesson1_lists > ul");
